...
| Research follow-on.1 | [02 — Conformance bundle](prds/02-conformance-bundle.md) | Freeze fixtures only after 08 and reuse 10 identity; benchmark native throughput |
| Research follow-on.2 | [03 — JAX wind](prds/03-jax-wind.md) | Wind-only verification pilot after 02; not a full sailing backend |
...
This revision is an authorized **planning update**, not a claim that runtime work or numerical validation has happened. All unshipped sections are proposed. In particular, 02–03 are not “dispatchable now” while their own normative deltas remain open. [brief.md](brief.md) records scope proposals; [00-foundations.md](00-foundations.md) records explicit model/API deltas. No signature or numerical approval is inferred from drafting these notes.
...
The **implemented** gate has nine steps: fmt, clippy, physics tests, invariant/convergence/provenance tests, regression, WASM build, TypeScript checking, web unit tests and browser E2E. Section 01 added the unit-test step. See the actual `scripts/check.sh` and `scripts/check.ps1` for commands.
...
| V-E | 02 adds conformance to step 4; 03 adds Python checks as steps 10–11; 04–06 extend Rust crate coverage when built. Do not install those gates before their tools exist |
...
`conformance.md` and `throughput.md` are **planned generated outputs, not present validation reports**. Implementation handoffs belong in `progress/`; this planning change creates no completion handoffs.
